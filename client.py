...
def rollDice():
    ...
    if playerType == 'player_1':
        SERVER.send(f'{value} player two turn'.encode())
    if playerType == 'player_2':
        SERVER.send(f'{value} player one turn'.encode())
...
def leftBoard():
    ...
    for i in range(0,11):
        if i == 0:
            ...
            boxLabel.place(x=x_pos,y=screen_height/2 -88)
...
def gameWindow():
    ...
    if playerType == 'player_1' and playerTurn:
